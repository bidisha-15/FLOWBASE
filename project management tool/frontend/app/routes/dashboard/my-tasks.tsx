...
const MyTasks = () => {
    ...
    const {data: myTasks, isLoading, error} = useGetMyTasksQuery() as {
        data: Task[] | undefined;
        isLoading: boolean;
        error: any;
    }
    ...
    const filteredTasks = myTasks && myTasks.length > 0 ? myTasks
      .filter((task) => task && task.project) // Filter out tasks with null projects
      .filter((task) => {
                  ...
                  <div key={task._id} className="p-4 hover:bg-muted/50">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-3">
                      <div className="flex">
                        ...
                        <div>
                          <Link
                            to={task.project?.workspace && task.project?._id ? 
